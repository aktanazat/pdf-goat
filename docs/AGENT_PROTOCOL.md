...
On unchanged files, `text`, `count`, `search`, and `compare text` read and fill a
disposable per-page cache. `text --layout` always runs live. `--no-cache` bypasses
both reads and writes.
`redact` never reads or writes the text cache; its hit finding always runs live.
...
A search reads the page count from the text cache, so a damaged count shortens
the hit list, and a limit moves `truncated` in either direction. A count above
the real one reports `truncated` true where a full read reports false. A count
below it, left consistent with the pages a `--pages` run primed, reports
`truncated` false on a limited search a full read would call incomplete, and
an unlimited search over the same store also reports false with a short hit
