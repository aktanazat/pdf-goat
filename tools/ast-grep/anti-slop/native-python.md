...
The pack contains all 13 assigned Python rules. This repository does not include the upstream fixture suite or lead report.
...
- **`no-dynamic-attribute-access`** flags `getattr` and `setattr` when the name argument is a string literal. It also flags `getattr(obj, "field", default)`. Calls with a dynamic name are not flagged.
